--------------------------------------------
--Created by OmerB, do not redistribute--
--------------------------------------------
...
cfg.blackjackTables = {
    ...
    [4] = {
        dealerPos = vector3(1143.96,263.66,-51.84),
        dealerHeading = 41.82,
        tablePos = vector3(1143.65, 264.34, -51.34),
        tableHeading = 163.32,
        distance = 1000.0,
        prop = "vw_prop_casino_blckjack_01b"
    },
            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Press ~INPUT_PICKUP~ to play the ~p~blackjack")
                    else 
...
Citizen.CreateThread(function()
    while true do 
        if waitingForBetState then
            ...
            if IsControlPressed(0, 38) then --Increase bet [pageup]
                currentBetAmount = currentBetAmount + 30
            end            
            if IsControlPressed(0, 44) then --Decrease bet [pagedown]
                if currentBetAmount >= 30 then 
                    currentBetAmount = currentBetAmount - 30
                end
            end            
        end
...
function setupBlackjackInstructionalScaleform(scaleform)
    ...
    ButtonMessage("Leave table (BACKSPACE)") --BACKSPACE
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(0)
    Button(GetControlInstructionalButton(2, 191, true))
    ButtonMessage("Place bet (ENTER)") --ENTER
    PopScaleformMovieFunctionVoid()
...
function setupBlackjackMidBetScaleform(scaleform)
    ...
    PushScaleformMovieFunctionParameterInt(1)
    Button(GetControlInstructionalButton(2, 194, true)) -- The button to display
    ButtonMessage("Stand (BACKSPACE)") --BACKSPACE
    PopScaleformMovieFunctionVoid()

    PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
    PushScaleformMovieFunctionParameterInt(4)
    Button(GetControlInstructionalButton(2, 22, true))
    ButtonMessage("Hit (Space)") --Space
    PopScaleformMovieFunctionVoid()  
